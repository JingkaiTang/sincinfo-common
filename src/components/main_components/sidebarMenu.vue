<template>
  <div class="sc-sidebar-menu">
    <div v-for="(l1, i1) in menuList">
      <div :class="['sc-menu-item', 'sc-menu-item-l1', {'sc-menu-item-active': i1 === active.l1}]" @click="menuClick(l1, {l1: i1})">
        <Icon :type="l1.icon"></Icon>
        <span class="sc-menu-text">l1: {{l1.name}}</span>
        <MenuArrow v-if="l1.children && l1.children.length > 0" :open="l1.$subOpen" @click="toggleSubmenu(l1)"></MenuArrow>
      </div>
      <div v-if="l1.children && l1.children.length > 0" v-show="l1.$subOpen">
        <div v-for="(l2, i2) in l1.children">
          <div :class="['sc-menu-item', 'sc-menu-item-l2', {'sc-menu-item-active': i2 === active.l2}]" @click="menuClick(l2, {l1: i1, l2: i2})">
            <Icon :type="l2.icon"></Icon>
            <span class="sc-menu-text">l2: {{l2.name}}</span>
            <MenuArrow v-if="l2.children && l2.children.length > 0" :open="l2.$subOpen" @click="toggleSubmenu(l2)"></MenuArrow>
          </div>
          <div v-if="l2.children && l2.children.length > 0" v-show="l2.$subOpen">
            <div v-for="(l3, i3) in l2.children">
              <div :class="['sc-menu-item', 'sc-menu-item-l3', {'sc-menu-item-active': i3 === active.l3}]" @click="menuClick(l3, {l1: i1, l2: i2, l3: i3})">
                <span class="sc-menu-text">l3: {{l3.name}}</span>
                <MenuArrow v-if="l3.children && l3.children.length > 0" :open="l3.$subOpen" @click="toggleSubmenu(l3)"></MenuArrow>
              </div>
              <div v-if="l3.children && l3.children.length > 0" v-show="l3.$subOpen">
                <div v-for="(l4, i4) in l3.children">
                  <div :class="['sc-menu-item', 'sc-menu-item-l4', {'sc-menu-item-active': i4 === active.l4}]" @click="menuClick(l4, {l1: i1, l2: i2, l3: i3, l4: i4})">
